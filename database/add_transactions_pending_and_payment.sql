...
ALTER TABLE `transactions`
  ADD COLUMN `amount_tendered` DECIMAL(38,16) NULL DEFAULT NULL AFTER `total_amount`,
  ADD COLUMN `change_amount` DECIMAL(38,16) NULL DEFAULT NULL AFTER `amount_tendered`,
  ADD COLUMN `payment_method` VARCHAR(30) NULL DEFAULT NULL AFTER `change_amount`,
  ADD COLUMN `amount_paid` DECIMAL(38,16) NULL DEFAULT NULL AFTER `payment_method`,
  ADD COLUMN `refunded_amount` DECIMAL(38,16) NOT NULL DEFAULT 0 AFTER `amount_paid`,
  ADD COLUMN `added_paid_amount` DECIMAL(38,16) NOT NULL DEFAULT 0 AFTER `refunded_amount`,
  ADD COLUMN `original_total_amount` DECIMAL(38,16) NULL DEFAULT NULL AFTER `added_paid_amount`,
